...
def doDistBinocularTask(ID=None, hemifield=None, location=None):
    ...
    x = 1
    et_filename = 'dstB' + ('LH' if hemifield == 'left' else 'RH')
    while len(glob(eyetracking_path + et_filename + str(x) + '.*')):
        x += 1

    # get everything shared from central:
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename=et_filename+str(x), task='distBinocular', ID=ID) # data path is for the mapping data, not the eye-tracker data!
    # setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename=et_filename+str(x), task='distBinocular', ID=ID, noEyeTracker=True) # data path is for the mapping data, not the eye-tracker data!
    ...
    ## stimuli
    point_1 = visual.Circle(win, radius = .5, pos = pol2cart(00, 3), units = 'deg', fillColor = col_binoc, lineColor = None)
    point_2 = visual.Circle(win, radius = .5, pos = pol2cart(00, 6), units = 'deg', fillColor = col_binoc, lineColor = None)
    point_3 = visual.Circle(win, radius = .5, pos = pol2cart(45, 3), units = 'deg', fillColor = col_binoc, lineColor = None)
    point_4 = visual.Circle(win, radius = .5, pos = pol2cart(45, 6), units = 'deg', fillColor = col_binoc, lineColor = None)
    ...
    left_pos_array_bsa  = pos_array[0:2]
    right_pos_array_bsa = pos_array[2:4]
    left_pos_array_out  = pos_array[4:6]
    right_pos_array_out = pos_array[6:8]
    ...
    foil_type = [1, -1] * 4
    hemifields = ['left', 'left', 'right', 'right'] * 2
    # pos_arrays = [pos_array_bsa[:]] * 4 + [pos_array_out[:]] * 4
    pos_arrays = [left_pos_array_bsa[:]] * 2 + [right_pos_array_bsa[:]] * 2 + [left_pos_array_out[:]] * 2 + [right_pos_array_out[:]] * 2
    ...
    while any(stairs_ongoing):
        ...
        point_1.fillColor = col_binoc
        point_2.fillColor = col_binoc
        point_3.fillColor = col_binoc
        point_4.fillColor = col_binoc
