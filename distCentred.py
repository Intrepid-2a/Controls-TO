...
def doDistCentredTask(ID=None, hemifield=None, location=None):
    ...
    filename = ID + '_distc_' + ('LH' if hemifield == 'left' else 'RH') + '_'
    ...
    x = 1
    et_filename = 'dstC' + ('LH' if hemifield == 'left' else 'RH')
    while len(glob(eyetracking_path + et_filename + str(x) + '.*')):
        x += 1

    # get everything shared from central:
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename=et_filename+str(x), task='distCentred', ID=ID) # data path is for the mapping data, not the eye-tracker data!
