...
def doDistAsynchronousTask(ID=None, hemifield=None, location=None):
    ...
    # ## path
    main_path = '../data/distAsynchronousNAM/'
    data_path = main_path
    ...
    filename = ID + '_distas_' + ('LH' if hemifield == 'left' else 'RH') + '_'
    ...
    x = 1
    et_filename = 'dsAS' + ('LH' if hemifield == 'left' else 'RH')
    while len(glob(eyetracking_path + et_filename + str(x) + '.*')):
        x += 1

    # get everything shared from central:
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename=et_filename+str(x), task='distAsynchronousNAM', ID=ID) # data path is for the mapping data, not the eye-tracker data!
