...
def doDistBinocHorizontalTask(ID=None, hemifield=None, location=None, addAsynchronous=True):
    ...
    # ## path
    main_path = '../data/distBinocHorizontal/'
    data_path = main_path
    ...
    filename = ID + '_distbh_' + ('LH' if hemifield == 'left' else 'RH') + '_'
    ...
    x = 1
    et_filename = 'dsBH' + ('LH' if hemifield == 'left' else 'RH')
    while len(glob(eyetracking_path + et_filename + str(x) + '.*')):
        x += 1

    # get everything shared from central:
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename=et_filename+str(x), task='distBinocHorizontal', ID=ID) # data path is for the mapping data, not the eye-tracker data!
    ...
    while any(stairs_ongoing):
        ...
        if which_first == 'Targ':
            point_1.pos = (positions[pos[0]][0]               + shift[0] -(tar/2)*dir, positions[pos[0]][1]) # target 1
            point_2.pos = (positions[pos[0]][0]               + shift[0] +(tar/2)*dir, positions[pos[0]][1]) # target 2
            point_3.pos = (positions[pos[1]][0] - (dif*dir)/2 + shift[1] -(tar/2)*dir, positions[pos[1]][1]) # foil 1
            point_4.pos = (positions[pos[1]][0] + (dif*dir)/2 + shift[1] +(tar/2)*dir, positions[pos[1]][1]) # foil 2
        else:
            point_3.pos = (positions[pos[0]][0]               + shift[0] -(tar/2)*dir, positions[pos[0]][1]) # target 1
            point_4.pos = (positions[pos[0]][0]               + shift[0] +(tar/2)*dir, positions[pos[0]][1]) # target 2
            point_1.pos = (positions[pos[1]][0] - (dif*dir)/2 + shift[1] -(tar/2)*dir, positions[pos[1]][1]) # foil 1
            point_2.pos = (positions[pos[1]][0] + (dif*dir)/2 + shift[1] +(tar/2)*dir, positions[pos[1]][1]) # foil 2
        ...
        if not gaze_out: # gaze_out is True when people stop fixating during stimulus presentation...
            ...
            position[which_stair] = position[which_stair] + [pos]     # this happens for ANY type of aborted trial...
            increment = False
            resp = 'auto_abort'
