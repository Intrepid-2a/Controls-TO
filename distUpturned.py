...
def doDistUpturnedTask(ID=None, hemifield=None, location=None):
    ...
    filename = ID + '_distt_' + ('LH' if hemifield == 'left' else 'RH') + '_'
    ...
    x = 1
    et_filename = 'dstT' + ('LH' if hemifield == 'left' else 'RH')
    while len(glob(eyetracking_path + et_filename + str(x) + '.*')):
        x += 1

    # get everything shared from central:
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename=et_filename+str(x), task='distUpturned', ID=ID) # data path is for the mapping data, not the eye-tracker data!
    ...
    ## prepare trials
    positions = {
        "left-top": [( spot_left[0] + (-1 *  ang_up_left) - 90,  spot_left[1] -  tar_left/2), ( spot_left[0] + (-1 *  ang_up_left) - 90,  spot_left[1] +  tar_left/2)],
        "left-mid": [( spot_left[0] + ( 0 *  ang_up_left) - 90,  spot_left[1] -  tar_left/2), ( spot_left[0] + ( 0 *  ang_up_left) - 90,  spot_left[1] +  tar_left/2)],
        "left-bot": [( spot_left[0] + ( 1 *  ang_up_left) - 90,  spot_left[1] -  tar_left/2), ( spot_left[0] + ( 1 *  ang_up_left) - 90,  spot_left[1] +  tar_left/2)],
        "righ-top": [(spot_right[0] + ( 1 * ang_up_right) + 90, spot_right[1] - tar_right/2), (spot_right[0] + ( 1 * ang_up_right) + 90, spot_right[1] + tar_right/2)],
        "righ-mid": [(spot_right[0] + ( 0 * ang_up_right) + 90, spot_right[1] - tar_right/2), (spot_right[0] + ( 0 * ang_up_right) + 90, spot_right[1] + tar_right/2)],
        "righ-bot": [(spot_right[0] + (-1 * ang_up_right) + 90, spot_right[1] - tar_right/2), (spot_right[0] + (-1 * ang_up_right) + 90, spot_right[1] + tar_right/2)],
    }
