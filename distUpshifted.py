...
def doDistUpshiftedTask(ID=None, hemifield=None, location=None):
    ...
    filename = ID + '_distu_' + ('LH' if hemifield == 'left' else 'RH') + '_'
    ...
    x = 1
    et_filename = 'dstM' + ('LH' if hemifield == 'left' else 'RH')
    while len(glob(eyetracking_path + et_filename + str(x) + '.*')):
        x += 1

    # get everything shared from central:
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename=et_filename+str(x), task='distUpshifted', ID=ID) # data path is for the mapping data, not the eye-tracker data!
    ...
    ## prepare trials
    positions = {
        "left-top": [( spot_left[0] + (-1 *  ang_up_left) - 0,  spot_left[1] -  tar_left/2), ( spot_left[0] + (-1 *  ang_up_left) - 0,  spot_left[1] +  tar_left/2)],
        "left-mid": [( spot_left[0] + ( 0 *  ang_up_left) - 0,  spot_left[1] -  tar_left/2), ( spot_left[0] + ( 0 *  ang_up_left) - 0,  spot_left[1] +  tar_left/2)],
        "left-bot": [( spot_left[0] + ( 1 *  ang_up_left) - 0,  spot_left[1] -  tar_left/2), ( spot_left[0] + ( 1 *  ang_up_left) - 0,  spot_left[1] +  tar_left/2)],
        "righ-top": [(spot_right[0] + ( 1 * ang_up_right) + 0, spot_right[1] - tar_right/2), (spot_right[0] + ( 1 * ang_up_right) + 0, spot_right[1] + tar_right/2)],
        "righ-mid": [(spot_right[0] + ( 0 * ang_up_right) + 0, spot_right[1] - tar_right/2), (spot_right[0] + ( 0 * ang_up_right) + 0, spot_right[1] + tar_right/2)],
        "righ-bot": [(spot_right[0] + (-1 * ang_up_right) + 0, spot_right[1] - tar_right/2), (spot_right[0] + (-1 * ang_up_right) + 0, spot_right[1] + tar_right/2)],
    }
    ...
    if hemifield == 'left':
        ...
        uppos = pol2cart( spot_left[0] - 90,  spot_left[1] )
        bppos = left_prop['cart']
        ...
        uppos = pol2cart( spot_right[0] + 90, spot_right[1] )
        bppos = right_prop['cart']


    point_offset = [uppos[x] - bppos[x] for x in range(2)]
