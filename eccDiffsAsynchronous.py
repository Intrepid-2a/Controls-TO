...
import pandas as pd # pandas to read txt / csv files
...
def getAsynchronouseRunDistanceDifferences(ID=None, hemifield=None, location=None, runtrials=0, log=None):
    ...
    recoveredFileName = ID + '_dista_' + ('LH' if hemifield == 'left' else 'RH') + '_1.txt'
    ...
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename='NONE_1', task='distAsynchronous', ID=ID) # data path is for the mapping data, not the eye-tracker data!
    ...
    rec_path = '../data/recDistAsynchronous/'
    recoveredFileName
