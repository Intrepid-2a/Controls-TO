...
def getUpScaledAsynchronousRunDistanceDifferences(ID=None, hemifield=None, location=None, runtrials=0, log=None):
    ...
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename='NONE_1', task='distUpScaledAsynchronous', ID=ID) # data path is for the mapping data, not the eye-tracker data!
    ...
    right_theta, right_radius = spot_right_ecc[0], spot_right_ecc[1]

    # let's respawn at the right position
    # size of blind spot + 2 (dot width, padding)
    if spot_left_cart[1] < 0:
        ang_up_left = (cart2pol(spot_left_cart_ecc[0], spot_left_cart_ecc[1] - size_left[1])[0] - spot_left_ecc[0]) + 2
    else:
        ang_up_left = (cart2pol(spot_left_cart_ecc[0], spot_left_cart_ecc[1] + size_left[1])[0] - spot_left_ecc[0]) + 2

    ang_up_right = (cart2pol(spot_right_cart_ecc[0], spot_right_cart_ecc[1] + size_right[1])[0] - spot_right_ecc[0]) + 2
    ...
    for trial_idx in range(responses.shape[0]):
        ...
        # instead of running the trial to get a response, we read it from the data file:
    ...
    rec_path = '../data/recDistUpScaledAsynchronous/'
    recoveredFileName
